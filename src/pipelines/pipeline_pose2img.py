...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def prepare_latents_individual(
        self,
        batch_size,
        num_channels_latents,
        width,
        ...
        if latents is None:
            ...
            for _ in range(batch_size):
                ...
                # scale the initial noise by the standard deviation required by the scheduler
                latents = latents * self.scheduler.init_noise_sigma
                out_tensors.append(latents)
            out_tensor = torch.concat(out_tensors, dim=0)
        else:
            latents = latents.to(device)
            out_tensor = latents
    ...
    def __call__(
        ...
        **kwargs,
        ...
        latents_init = self.prepare_latents_individual(
        ...
        latents = latents_init.unsqueeze(2)  # (bs, c, 1, h', w')
        ...
        return Pose2ImagePipelineOutput(images=image), latents_init
