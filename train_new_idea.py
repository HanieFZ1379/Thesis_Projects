...
from diffusers import AutoencoderKL, DDPMScheduler, DDIMScheduler, DPMSolverMultistepScheduler, UniPCMultistepScheduler
...
# def generate_clip_random_noise(bs, device, dtype, scale_factor=0.05, use_dino=False):
#     values = []
#     if use_dino:
#         std = np.load('./data/dino_std.npy')  # فایل جدید برای DINO
#     else:
#         std = np.load('./data/clip_std.npy')  # فایل قدیمی برای CLIP
#     for _ in range(bs):
#         tmp = torch.normal(mean=0, std=torch.tensor(std))
#         tmp = tmp.to(device=device, dtype=dtype)
#         values.append(tmp)
...
class InstanceNet(nn.Module):
    def __init__(
        ...
        self.device = device
        # New: Projector for DINO embeds (1024 -> cross_attention_dim=768)
        weight_dtype = 'fp16'
        self.clip_projector = nn.Linear(1024, 768).to(device=torch.device(self.device), dtype=torch.float16 if weight_dtype == 'fp16' else torch.float32)  # Cast device to torch.device
    ...
    def forward(
        ...
        if not uncond_fwd:
            ...
            # self.reference_unet.to(self.device)  # 🔁 bring it to GPU
            # with torch.no_grad():
            #     self.reference_unet(
            #         ref_image_latents,
            #         ref_timesteps,
        ...
        # print(face_tokens.shape, clip_image_embeds.shape, clip_image_embeds2.shape)
...
# This function computes the Signal-to-Noise Ratio (SNR) based on the current timestep and the noise scheduler.
...
def log_validation(
    ...
    # changes
    # ساخت متریک
    fid_metric = build_metric().to(accelerator.device)
    ...
    texture_clip_condition=data_config.texture_clip_condition # equal to False in config file
    texture_unet_condition=data_config.texture_unet_condition # equal to to True in config file
    width=data_config.train_width #512
    height=data_config.train_height #512
    bs = data_config.val_bs # 8

    generator = torch.manual_seed(42)
    # cast unet dtype
    vae = vae.to(dtype=torch.float32)
    image_enc = image_enc.to(dtype=torch.float32) # clip image encoder
    ...
    for jdx in range(bs):
        ...
        ref_image_pil = to_pil(batch['ref_img'][jdx]) # texture map image
        ...
        image = pipe(
            ref_image_pil, # texture map image
            pose_image_pil, # target densepose
            width,
            height,
            20,
            3.5,
            generator=generator,
            clip_image=clip_image_pil, # Iemb
            clip_image2=clip_image_pil2, # Aemb
            face_emb=face_embs, # Femb
        ).images
        ...
        # ذخیره برای متریک
        # اضافه کردن به لیست برای متریک
        gt_tensor = transforms.ToTensor()(gt_image_pil).unsqueeze(0).to(accelerator.device)
        pred_tensor = torch.from_numpy(image).permute(2, 0, 1).unsqueeze(0).to(accelerator.device, dtype=torch.float32)

   
        pred_images.append(pred_tensor)  # پیش‌بینی مدل
        gt_images.append(gt_tensor)      # تصویر واقعی
        ...
        del image, res_image_pil_1, image_out  # Free temporary tensors/images

    # محاسبه متریک‌ها
    all_gt = torch.cat(gt_images, dim=0)
    all_pred = torch.cat(pred_images, dim=0)

    fid_features_real = fid_metric(all_gt, None)
    fid_features_fake, lpips, psnr, ssim, ssim_256 = fid_metric(all_gt, all_pred)
    fid_score = compute_fid_deepfashion(fid_features_real, fid_features_fake)

    # fid_score = compute_fid_from_features()

    logger.info(f"[Validation @ step {num_iters}] "
                f"FID: {fid_score:.4f} | LPIPS: {lpips.mean().item():.4f} | "
                f"PSNR: {psnr.mean().item():.4f} | SSIM: {ssim.mean().item():.4f}")
...
def compute_fid_deepfashion(fid_features_real, fid_features_fake):
    # اطمینان از اینکه شکل درست است
    if isinstance(fid_features_real, torch.Tensor):
        fid_features_real = fid_features_real.cpu().numpy()
    if isinstance(fid_features_fake, torch.Tensor):
        fid_features_fake = fid_features_fake.cpu().numpy()

    mu1 = np.mean(fid_features_real, axis=0)
    sigma1 = np.cov(fid_features_real, rowvar=False)
    mu2 = np.mean(fid_features_fake, axis=0)
    sigma2 = np.cov(fid_features_fake, rowvar=False)
...
def main(cfg):
    ...
    accelerator = Accelerator(
        gradient_accumulation_steps=cfg.solver.gradient_accumulation_steps, # 1
        mixed_precision=cfg.solver.mixed_precision, # fp16
        log_with="mlflow",
        project_dir="./mlruns",
        cpu=(cfg.device == 'cpu'),
    ...
    # حذف پارامترهای ناسازگار با DPMSolverMultistepScheduler
    # sched_kwargs.pop("clip_sample", None)
    # sched_kwargs.pop("rescale_betas_zero_snr", None)
    val_noise_scheduler = DDIMScheduler(**sched_kwargs)
    sched_kwargs.update({"beta_schedule": "scaled_linear"})
    train_noise_scheduler = DDIMScheduler(**sched_kwargs)
    # val_noise_scheduler = DPMSolverMultistepScheduler(**sched_kwargs)
    ...
    for name, module in reference_unet.named_modules():
        if isinstance(module, torch.nn.Linear) and "up_blocks.3" in name:
    ...
    reference_unet = get_peft_model(reference_unet, lora_config)

    # 3. Load only LoRA weights from saved checkpoint
    lora_state_dict = torch.load("./checkpoints/reference_unet_lora.pth", map_location="cpu")

    # 4. Apply only LoRA weights
    reference_unet.load_state_dict(lora_state_dict, strict=False)

    # # 5. Move only LoRA modules to GPU (same as قبل)
    # for name, module in reference_unet.named_modules():
    #     if 'lora_' in name:
    ...
    # reference_unet = UNet2DConditionModel.from_pretrained(cfg.base_model_path, subfolder="unet").to(cfg.device)
    # reference_unet.load_state_dict(torch.load("./checkpoints/reference_unet.pth", map_location='cpu'), strict=False)
    ...
    if cfg.pose_guider_pretrain: # True
        pose_guider = PoseGuider(conditioning_embedding_channels=320, block_out_channels=(16, 32, 96, 256), conditioning_channels=cfg.cond_chan).to(cfg.device)
        pose_guider.load_state_dict(torch.load("./checkpoints/pose_guider.pth", map_location="cpu"),
            strict=False,)
        # pose_guider.load_state_dict(torch.load("./checkpoints/pose_guider.pth"))
    else:
        pose_guider = PoseGuider(
    ...
    for name, param in reference_unet.named_parameters():
        if "lora_" in name:
            if "up_blocks.3" in name:
                ...
                param.requires_grad_(True)
    # Freeze all parameters of reference_unet
    # for param in reference_unet.parameters():
    #     param.requires_grad_(False)
    ...
    if cfg.solver.enable_xformers_memory_efficient_attention: # enable_xformers_memory_efficient_attention = False
    ...
    if cfg.solver.gradient_checkpointing: # gradient_checkpointing = False
    ...
    image_enc = Dinov2Model.from_pretrained("facebook/dinov2-large").to(dtype=weight_dtype, device=cfg.device)
    image_enc.requires_grad_(False)
    image_proj_model.requires_grad_(True)
    if cfg.solver.scale_lr: # False
        learning_rate = (
            cfg.solver.learning_rate
            * cfg.solver.gradient_accumulation_steps # 1
            * cfg.data.train_bs # 12
    ...
    if cfg.solver.use_8bit_adam: # False
        try:
    ...
    # train_dataset, val_dataset = get_deepfashion_dataset(cfg, double_clip=True, use_face_emb=True, subset_size=1)
    train_dataset, val_dataset = get_deepfashion_dataset(cfg, double_clip=True, use_face_emb=True, subset_size=1, use_dino=True)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                with torch.no_grad():
                    ...
                    dino_outputs = image_enc(clip_img.to(cfg.device, dtype=weight_dtype))
                    clip_image_embeds = dino_outputs.last_hidden_state.mean(dim=1)  # [bs, 1024]
                    # Project to 768
                    clip_image_embeds = net.clip_projector(clip_image_embeds)  # [bs, 768]
                    ...
                    dino_outputs2 = image_enc(clip_img2.to(cfg.device, dtype=weight_dtype))
                    clip_image_embeds2 = dino_outputs2.last_hidden_state.mean(dim=1)
                    # Project to 768
                    clip_image_embeds2 = net.clip_projector(clip_image_embeds2)  # [bs, 768]
                    image_prompt_embeds2 = clip_image_embeds2.unsqueeze(1)  # [bs, 1, 768]
                ...
                lr_scheduler.step()
                optimizer.zero_grad()
                # Clear GPU memory to avoid memory fragmentation after the optimizer step
                torch.cuda.empty_cache()  # This will free up unused memory
            if accelerator.sync_gradients:
                ...
                if (global_step - 1) % cfg.val.validation_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        # changes
                        # لاگ pose_scale هر 1000 استپ
                        if global_step % 1000 == 0:
                            try:
                                pose_scale_val = accelerator.unwrap_model(net).denoising_unet.pose_scale.item()
                                logger.info(f"[Step {global_step}] Pose scale: {pose_scale_val:.4f}")
                                accelerator.log({"pose_scale": pose_scale_val}, step=global_step)
                            except AttributeError:
                                logger.warning("pose_scale not found in model.")
